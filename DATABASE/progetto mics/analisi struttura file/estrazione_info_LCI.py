...
def extract_elementary_exchange(elementary_exchanges):
    ...
    for exchange in elementary_exchanges:
        ...
        output.append(f"       - Elementary Exchange ID: {exchange.get('@elementaryExchangeId', 'N/A')}")
        output.append(f"       - Name: {exchange.get('name', {}).get('#text', 'N/A')}")
        output.append(f"       - Amount: {exchange.get('@amount', 'N/A')}")
        output.append(f"       - Unit ID: {exchange.get('@unitId', 'N/A')}")
        ...
        output.append(f"       - Compartment: {compartment}")
        output.append(f"       - Subcompartment: {subcompartment}")
    return "\n".join(output)
...
def main():
    ...
    with open("progetto mics/analisi struttura file/extracted_info_lci.txt", "w") as output_file:
