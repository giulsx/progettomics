...
# Carica il file CFs.json contenente i coefficienti di caratterizzazione
with open('progetto mics/CFs.json', 'r', encoding='utf-8') as file:
    cfs_data = json.load(file)

# Funzione per trovare CFs corrispondenti all'elementaryExchange in base al nome
def find_cfs_for_exchange(exchange_name, cfs_data):
    return [cf for cf in cfs_data if cf['Name'] == exchange_name]

# Funzione per estrarre i dati dal file UPR e aggiungere CFs agli elementaryExchanges
def extract_from_upr_with_cfs(upr_data, cfs_data):
    data = upr_data
...
# Estrarre Commenti Generali con gestione dei casi nulli o mancanti
    general_comments = []

# Inizializzazione di general_comment_texts
    general_comment_texts = None

# Verifica se 'generalComment' esiste
    if 'generalComment' in activity:
    # Ottieni il contenuto di 'text' da 'generalComment'
        general_comment_texts = activity['generalComment'].get('text')

# Processa i commenti solo se 'general_comment_texts' è stato definito
    if general_comment_texts is not None:
    # Se 'text' è un dizionario con il campo '#text'
    ...
    # Se 'text' è una lista di dizionari
...
# Altrimenti, general_comments rimane una lista vuota
...
# Estrarre Elementary Exchanges con CFs
    elementary_exchanges = []

    # Controlla se esistono gli elementary exchanges
    elementary_exchange_data = upr_data['ecoSpold']['childActivityDataset']['flowData'].get('elementaryExchange', [])

    # Se elementaryExchange è un singolo dizionario, lo trasformiamo in una lista
    if isinstance(elementary_exchange_data, dict):
        elementary_exchange_data = [elementary_exchange_data]

    # Ora, elementary_exchange_data è sempre una lista
    for exchange in elementary_exchange_data:
        
        # Gestisce il campo 'name' che può essere un dizionario o una stringa
        if isinstance(exchange['name'], dict):
            exchange_name = exchange['name'].get('#text', 'N/A')  # Usa '#text' se è un dizionario
        else:
            exchange_name = exchange['name']  # Se è una stringa, usala direttamente
        
        # Gestisce il campo 'unitName' che può essere un dizionario o una stringa
        if isinstance(exchange['unitName'], dict):
            unit_name = exchange['unitName'].get('#text', 'N/A')  # Usa '#text' se è un dizionario
        else:
            unit_name = exchange['unitName']  # Se è una stringa, usala direttamente
        
        # Trova i coefficienti di caratterizzazione (CFs) per l'exchange
        cf_list = find_cfs_for_exchange(exchange_name, cfs_data)
        
        # Aggiungi l'exchange alla lista
...
def write_to_json_with_cfs(upr_data, lcia_data, cfs_data, output_file):
    upr_extracted_data = extract_from_upr_with_cfs(upr_data, cfs_data)
...
for url in urls:
    ...
    output_file = output_dir / f"output_with_cfs_{dataset_id}.json"

    # Scrivi i dati estratti nel file JSON
    write_to_json_with_cfs(json_data_upr, json_data_lcia, cfs_data, output_file)
