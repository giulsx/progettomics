        ...
        if isinstance(json_data, dict):
            for key, value in json_data.items():
                if isinstance(value, (str, int)) and str(value) == str(id_value):
                    ...
                    elif key == "@impactMethodId":
                        # Se l'ID è un subcompartmentId, cerca il nome del compartment
                        impactmethod_name = json_data.get("impactMethodName", "Unknown")
                        return {
                            "key": key,
                            "unit_name": impactmethod_name,
                        }
                    elif key == "@reviewerId":
...
# Carica il file JSON
json_file_path = "progetto mics/ecoinvent-3.10-cutoff-lcia-15352.json"
try:
...
id_file_path = "progetto mics/analisi id/output_id_counts_greater_than_one_LCIA.txt"
...
output_file_path = "progetto mics/analisi id/output_id_texts_LCIA.txt"
try:
